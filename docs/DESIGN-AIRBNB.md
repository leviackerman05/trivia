# Airbnb Design System (owner-supplied capture, 2026-08-05)

> Source: getdesign extraction of airbnb.com, pasted by the owner as the
> replacement direction for the design-merge experiment. This file is the
> verbatim source material; `docs/DESIGN-MERGE.md` (Cloudflare × Vercel
> direction) is **superseded** by this capture — keep both for provenance,
> implement THIS one. Status: capture only, no implementation yet.
...
**`property-card-photo`** — The photo plate itself, separated as a token because some surfaces (wishlist, search results) reuse just the photo without the photo meta block.
...
## Implementation status (design-airbnb branch, 2026-08-05)

Restyled per the owner-approved decisions block (2026-08-05) as a token-layer
rewrite + component-style updates only — no layout or markup restructuring.

**Implemented per token** (`src/styles/global.css`):

- Light-first `@theme` base with provenance comments per token (`[AIRBNB]` /
  `[OWNER]` / `[CARRY]`); dark theme kept as an optional `.dark` toggle,
  restyled on the same Airbnb tokens (ink `#222222` surfaces, white text,
  Rausch accents).
- Rausch `#ff385c` primary (white text, active `#e00b41`, disabled `#ffd1da`);
  orange retired. Text-on-light Rausch tokens darkened (`#c20a37` / `#b00533`)
  so they hold AA on the Rausch tint chips — Airbnb has no text-on-tint tokens.
- Type scale per decision 4 mapped onto the legacy h1–h4 utilities
  (28/700, 22/500, 21/700, 20/600), plus new `text-badge` (11/600) and
  `text-score` (64/700) tokens — the rating-display moment is used on the
  SoloShell results score.
- Radii ladder by value (controls 8, cards 14, 20, 32, full pills); buttons
  8px in the kit, pill only for badge/pill variants.
- Single elevation tier (hairline ring at rest + one hover-float shadow);
  50% scrim; 2px ink focus ring (16:1 light, inverted white on dark) replacing
  the emerald ring; status colors carried over per decision 6 (Airbnb error
  `#c13515` adopted for light-theme error text).
- Inter stays self-hosted; reduced-motion kill switch kept; content cap 1280px.

**Component-style updates** (`src/components/ui/*`): Button (Rausch primary,
8px, ink-outline secondary), Badge/Chip (pill, badge type), Card (flat +
hover-float), Input (2px ink focus, no glow), Dialog (50% scrim), EmptyState,
List, StatCard, PlayerCard, CategoryCard, LeaderboardTable (type scale, muted
tiers). Islands/pages: tokens only, plus the focus-ring sweep (emerald ring →
ink) and the SoloShell results score.

**One flagged deviation (owner decision 1, verbatim):** white text on
`#ff385c` CTAs measures 3.52:1 — the same pattern Airbnb ships, kept per the
confirmed decision; it surfaces as a single serious axe violation on
server-rendered CTA buttons (see axe summary in the PR). Remedies if a
stricter AA gate is ever wanted: fill `#e00b41` (4.9:1) or large-text labels
(20px bold, 3:1 exemption).

---

## Shape pass (design-airbnb, 2026-08-05) — implemented per mapping m1–m12

Owner verdict: "it only changed colors, not the shape and sizes — I want it to
actually look like the Airbnb website, somewhat, not exactly." This pass
rebuilds shapes and layout on top of the untouched token layer; every borrowed
element maps to a real TriviaHub function (no wishlists, reservations, or date
pickers).

- **m1 Nav shell** — 80px sticky white nav, hairline bottom, logo left,
  centered links with a 2px ink underline on `aria-current`, theme toggle +
  Rausch "Play now" pill on the right (OWNER a). Mobile tab bar: white,
  hairline top, ink active.
- **m2 Hero pill + orb** — segmented pill [Play solo | With friends | Room
  code] terminated by a 48px Rausch orb; segments route to `/#games`,
  `/#multiplayer`, and the room-code flow (`/game/skribbl-arena` lobby).
  Mobile: single tappable pill opens a full-screen picker (three 48px rows,
  Escape/close, OWNER b). **⚠ SUPERSEDED by the v3 pass below (owner verdict
  "still wrong")** — the pill, orb, and mobile picker were removed.
- **m3 Category strip** — circular emoji buttons + captions on the homepage
  hero and `/categories` (real links to `/#games-{family}`).
- **m4 Property cards** — GameCard/DailyCard: emoji plate (14px, soft fill),
  floating white pill badge, Rausch play orb, meta stack (title 16/600,
  tagline 14 muted, players · duration · energy + Free); hairline + single
  hover-float shadow. `games.json` gained an additive `emoji` field.
- **m5 Buttons** — 48px primary 16px w500, ink-outline secondary, tertiary
  (plain ink text, underline on hover), Rausch pill variant for featured CTAs.
- **m6 Inputs** — 56px, 8px radius, hairline, stacked muted caption label,
  2px ink focus (no glow); applied to the lobby nickname/room-code forms.
- **m7 Footer** — 3-column link lists (title-sm heads, body-sm ink links) +
  muted legal band with `#428bff`-family legal links (light theme uses the
  AA-adjusted `#2f6fe4`; token `--color-legal-link`).
- **m8 Score moments** — 64/700 rating-display applied to the SoloShell
  results score, TriviaSolo results, Charades game-end, and the daily hub's
  grand-streak stat.
- **m9 Room surfaces** — lobby cards (14px + hairline + single shadow); room
  actions become a sticky bottom bar on mobile (above the tab bar) and static
  in-flow on lg+.
- **m10/m11** — 64px section bands + 16px gutters + 1280px cap; single
  elevation tier only.
- **m12 Tokens** — token layer untouched; added `--color-legal-link` and the
  `pb-*` shape classes (plain CSS in `@layer components`, values resolve from
  the theme) so the repeated homepage markup stays cheap (PRD §10 budget).

**CTA fill note (shape-pass acceptance):** the axe gate re-flagged white on
`#ff385c` labels, so per the acceptance criteria all white-text CTA fills now
use the `#e00b41` active shade (4.9:1) at the component layer — `#ff385c`
remains the brand accent on non-text elements (orbs, play circles) and tints.
Axe is now 0 violations on `/`, `/daily`, `/game/trivia`.

---

## v3 pass (design-airbnb, 2026-08-05) — hero/nav/de-emoji + polish

Owner verdict: the v2 hero pill, nav, and emoji treatment are **rejected** —
"still wrong." v3 fixes on top: exactly two hero buttons, a nav that never
wraps, zero emoji in UI chrome (in-repo SVG line icons), and a page-by-page
polish sweep. Game behavior and the room API untouched.

- **Hero (2 buttons)** — pill, orb, and mobile picker deleted. `index.astro`
  renders exactly **Play** (primary Rausch, large) → `/games` and **Create
  room** (ink-outline secondary) → `/games?mode=room`; mobile stacks them.
- **New `/games` page** — SSG catalog of all 19 games reusing `GameCard`;
  family filter is static anchors (`?family=drawing|voting|solo|special|quiz`)
  applied by a tiny inline script on the static host (no client state);
  `?mode=room` hides solo games and re-points cards to `/game/[slug]?room=1`;
  added to `public/sitemap.xml` and `scripts/smoke.mjs`.
- **Room flag** — `[slug].astro` reads `?room=1` and passes `autoCreateRoom`
  to the lobby islands; `RoomLobbyPanel` calls the existing create-room
  action once on connect. Solo pages ignore the flag.
- **Nav no-wrap** — flex header, `whitespace-nowrap` labels, tighter
  `pb-nav-link` sizing; desktop links hidden below `lg` so the header never
  wraps at any breakpoint. Mobile tab bar labels nowrap. "Games" repointed
  to `/games`.
- **De-emoji (chrome)** — new in-repo icon set `src/lib/icons.ts` (32 stroke
  glyphs) + `Icon.astro`/`Icon.tsx` wrappers (`.pb-icon` CSS stroke
  defaults, cheap across the homepage). Replaced on: family sections
  (🎨🗳️🧩🎭📚 → pencil/ballot/user/confetti/question), GameCard plates
  (monogram tiles — the sanctioned family-icon-or-monogram alternative),
  CategoryStrip, daily registry (`'🧠'` → `'brain'` etc., field name
  unchanged), theme toggle (sun/moon), badges, results screens, footer.
  **Exempt:** `emoji-plots.json`, daily drawing/music clue emojis, and all
  puzzle-data emoji (game content; their tests untouched and green).
- **Polish sweep (fixed-items list)** — see the summary in the PR; key fixes:
  play-orb glyph as pure-CSS triangle (no inline SVG per card),
  `DailyArchive` renders registry icon keys via `Icon` (was raw text), nav
  underline/active states, 48px+ touch targets, 2px ink focus retained.
- **Budget** — homepage **99.5 KB** (< 100 KB gate); `/games` static + CSS
  icon set keep the new page cheap; island bundles within budget.
- **Gates** — `pnpm verify` green: format, lint, typecheck (0 errors),
  **187 client + 190 server tests**, both builds (43 pages incl. `/games`),
  smoke incl. budgets. One-off axe scan (local, CI untouched): **0
  violations** on `/`, `/daily`, `/game/skribbl-arena`.

**Design Lead review (2026-08-05): RETURN FOR CHANGES** — v3 state
approved as far as it goes (build green, homepage HTML 48.8 KB, axe 0 per
docs), but the Phase A scope is incomplete: R1 ("TriviaHub" copy still in
logo/legal/meta/JSON-LD/OG/404-500), R6 ("Daily Games" label), R3
(`voting`/`special` families), R13 (D10 template), R15 (streak box),
R2 (card images) are not on the branch. Binding specs + fix list:
`docs/DESIGN-STANDARD.md` (game-page standard, icon-system spec,
card-image direction, prioritized fix list). No preview URL exists for the
branch yet — push it so the next review gate can visit a preview.
